...
* 相机基础： 深度相机成像原理、深度噪声来源、有效测距范围、反光和强光对深度的影响。
* ROS2 基础： 节点、话题、TF、rosbag2 录制回放、launch、参数管理。
* 相机驱动与数据检查： 能稳定发布 RGB、Depth、CameraInfo，能看帧率、延迟、丢帧。
* 相机标定： 内参、外参、相机与车体坐标系关系，理解为什么标定误差会直接毁掉建图质量。
* RTAB-Map 基础： 能完成实时建图、离线重放、数据库查看、轨迹导出。

### 第二步

* 时间同步与坐标系统一: 相机、IMU、里程计的时间戳对齐，TF 树设计。
* 里程计与建图质量调试: 学会看失败模式：漂移、跳变、回环误检、纹理退化。
* 轮速计和 IMU 融合基础: 学会用 EKF 或 UKF 做里程估计融合，让相机在弱纹理时不至于完全失效。
* 地图表达: 2D 占据栅格和 3D 点云地图的区别、用途、存储成本。
* 计算资源优化: 分辨率、帧率、特征数量、CPU 占用之间的取舍。
