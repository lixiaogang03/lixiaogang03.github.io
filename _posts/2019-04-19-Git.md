...
# 1. 明确只跟踪当前分支
git remote set-branches origin main_plus_haoqiang_BF

# 2. 清理旧的远程跟踪分支（这一步会看到所有分支，但只做一次）
git fetch --prune

# 3. 验证配置
git remote show origin   # 确认 Tracked remote branches 只剩一个

# 4. 以后正常使用，只会更新这一个分支
git fetch                # 测试，应该只更新 main_plus_haoqiang_BF
git pull                 # 同上
...
# 步骤1：先拉取远程最新变更，并把你的本地提交“重放”到远程最新提交之上
git pull --rebase origin main_plus_haoqiang_BF
