...
scp -P 10357 root@ip.ah1.suanlix.cn:/root/code/sam3_test/object.jpg ./code/AI/

# 语法：scp [本地文件路径] [用户名]@[服务器IP]:[远程路径]
scp -P 10357 car_768x768.jpg root@ip.ah1.suanlix.cn:/root/code/sam3_test/
...
| 图像编码                          | 330ms     | ~36% |
| 文本检测                          | 399ms     | ~44% |
| 其他（mask decode + postprocess） | ~180ms    | ~20% |
| **总计**                        | **915ms** | 100% |

### object 对比 window

| prompt | mask 数量 | score分布        | 语义稳定性  |
| ------ | ------- | -------------- | ------ |
| window | 30      | 更集中（0.2~0.48）  | 🎯 较稳定 |
| object | 53      | 更分散（0.05~0.35） | ⚠️ 很发散 |
